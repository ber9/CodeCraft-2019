...
public class Graph {
    ...
    private HashMap<String,String> roadsId;

    public Graph() {
        nodes = new HashMap<String,Node>();
        roadsId = new HashMap<String,String>();
    }

    public Graph(String filename) {
        this();
        readFromNewFile(filename);
    }
    ...
    public void addEdge(String label1, String label2, Double weight, Road road) {
        if (!nodes.containsKey(label1))
            addNode(label1);
        if (!nodes.containsKey(label2))
            addNode(label2);
        nodes.get(label1).addEdge(label2,weight,road);
    }
    ...
    public void readFromNewFile(String fileName) {
        try {
            BufferedReader in = new BufferedReader(new FileReader(fileName));
            ...
            line = in.readLine();
            while (line != null) {
                String[] edgeDescription = line.substring(1, line.length() - 1).split(",");
                Road road = new Road(edgeDescription[0].trim(),Integer.parseInt(edgeDescription[1].trim()),
                ...
                if(edgeDescription[6].trim().equals("1")){
                    addEdge(edgeDescription[5].trim(),edgeDescription[4].trim(), null,road);
                }
                line = in.readLine();
            }
        } catch (Exception e) {
    ...
    public HashMap<String, String> getRoadsId() {
        return roadsId;
    }

    public void setRoadsId(HashMap<String, String> roadsId) {
