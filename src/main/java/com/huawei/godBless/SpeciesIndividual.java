...
public class SpeciesIndividual {
    ...
    List<Path> paths;//基因序列
    int totalTime;//路程
    int fitness;//适应度
    SpeciesIndividual next;
    float rate;

    SpeciesIndividual(List<Path> paths) {
        //初始化
        this.paths = paths;
        this.fitness = 0;
        this.totalTime = 0;
    ...
    /*void createByRandomGenes() {
        ...
        public SpeciesIndividual clone(){
            ...
            for(Path path:this.paths){
                newPaths.add(path.clone());
            }
            species.paths = newPaths;
            species.totalTime = this.totalTime;
            species.rate = this.rate;
